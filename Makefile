...
PROJECT_NAME ?= provider-upjet-iosxe
...
export TERRAFORM_PROVIDER_SOURCE ?= CiscoDevNet/iosxe
export TERRAFORM_PROVIDER_REPO ?= https://github.com/CiscoDevNet/terraform-provider-iosxe
export TERRAFORM_PROVIDER_VERSION ?= 1.0.0
export TERRAFORM_DOCS_PATH ?= docs/resources

# Terraform is only used at code generation time, to read the provider schema
# and scrape the provider documentation. This is a no-fork provider: the Cisco
# IOS-XE Terraform provider is linked into the controller binary and called
...
xpkg.build.provider-upjet-iosxe: do.build.images
